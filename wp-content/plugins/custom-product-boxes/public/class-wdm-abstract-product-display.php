...
(min-device-width: 768px) and (max-device-width: 1024px)');
    ...
    public function getStockMessage($singleProduct)
    {
        ...
        if (! $singleProduct->is_in_stock()) {
            ...
            $message = $singleProduct->backorders_require_notification() ? __('Available on backorder', 'custom-product-boxes') : __('Em estoque', 'custom-product-boxes');
            $availabilityText = $this->setAvailabilityInfo(
                $this->priceMessageType == 'html' ? $this->getHtmlPriceMessage($message) : $message
            );
            ...
            $message = __('Em estoque', 'custom-product-boxes');
            $availabilityText = $this->setAvailabilityInfo(
                $this->priceMessageType == 'html' ? $this->getHtmlPriceMessage($message) : $message,
                ''
            );
        ...
        switch (get_option('woocommerce_stock_format')) {
            ...
            case 'low_amount':
                ...
                if ($singleProduct->get_stock_quantity() <= get_option('woocommerce_notify_low_stock_amount')) {
                    $message = sprintf(__('Apenas %s disponíveis no estoque', 'custom-product-boxes'), $singleProduct->get_stock_quantity());
            ...
            default:
                $backorderClass = '';
                $message = sprintf(__('%s em estoque', 'custom-product-boxes'), $singleProduct->get_stock_quantity());

...
// Reset addons and nyp prefix
                    ...
                    if (isset($product_qty_in_cart[ $variation_id ]) && ! $product_data->has_enough_stock($product_qty_in_cart[ $variation_id ] - $qty_counted_in_cart + $quantity)) {
                        if (! $add) {
                            wdm_pb_bundles_add_notice(sprintf(
                                ...
                                sprintf(__('&quot;%s&quot; nâo pôde ser adicionado ao carrinho porque não há mais disponibilidade deste produto no estoque. Nós possuímos %s no estoque e você tem %s em seu carrinho.', 'custom-product-boxes'), $bundle->get_title(), $product_data->get_title(), $product_data->get_stock_quantity(), $product_qty_in_cart[ $variation_id ])
                            ), 'error');
                        }
                        return false;
                    }
                    ...
                    if (isset($product_qty_in_cart[ $product_id ]) && ! $product_data->has_enough_stock($product_qty_in_cart[ $product_id ] - $qty_counted_in_cart + $quantity)) {
                        if (! $add) {
                            wdm_pb_bundles_add_notice(sprintf(
                                '<a href="%s" class="button wc-forward">%s</a> %s',
                                $woocommerce->cart->get_cart_url(),
                                __('Ver Carrinho', 'custom-product-boxes'),
                                sprintf(__('&quot;%s&quot; nâo pôde ser adicionado ao carrinho porque não há mais disponibilidade deste produto no estoque. Nós possuímos %s no estoque e você tem %s em seu carrinho.', 'custom-product-boxes'), $bundle->get_title(), $product_data->get_title(), $product_data->get_stock_quantity(), $product_qty_in_cart[ $product_id ])
                            ), 'error');
                        }
                        return false;
                    }
                ...
                if (isset($product_qty_in_cart[ $product_id ]) && ($product_qty_in_cart[ $product_id ] == 1 && $quantity == 1)) {
                    if (! $add) {
                        wdm_pb_bundles_add_notice(sprintf(
                            ...
                            $woocommerce->cart->get_cart_url(),
                            __('Ver Carrinho', 'custom-product-boxes'),
                            sprintf(__('&quot;%s&quot; nâo pôde ser adicionado ao carrinho porque há apenas 1 unidade deste produto no estoque e você já o adicionou em seu carrinho.', 'custom-product-boxes'), $bundle->get_title(), $product_data->get_title())
                        ), 'error');
                    }
                    return false;
                }
    ...
    public function addNoticeConditionally($add, $type, $bundle, $product_data)
    {
        ...
        if (! $add) {
            if ($type == 'isi') {
                wdm_pb_bundles_add_notice(sprintf(__('&quot;%s&quot; não pôde ser adicionado ao carrinho &mdash; apenas 1 unidade pode sem adquirida.', 'custom-product-boxes'), $bundle->get_title(), $product_data->get_title()), 'error');
            } elseif ($type == 'is_pur') {
                wdm_pb_bundles_add_notice(sprintf(__('&quot;%s&quot; não pôde ser adicionado ao carrinho porque &quot;%s&quot; não pode ser adquirido.', 'custom-product-boxes'), $bundle->get_title(), $product_data->get_title()), 'error');
            } elseif ($type == 'instk') {
                wdm_pb_bundles_add_notice(sprintf(__('&quot;%s&quot; não pôde ser adicionado ao carrinho porque &quot;%s&quot; está fora do estoque.', 'custom-product-boxes'), $bundle->get_title(), $product_data->get_title()), 'error');
            } elseif ($type == 'hasstk') {
                wdm_pb_bundles_add_notice(sprintf(__('&quot;%s&quot; não pôde ser adicionado ao carrinho porque não há quantidade suficiente do &quot;%s&quot; no estoque (%s restantes).', 'custom-product-boxes'), $bundle->get_title(), $product_data->get_title(), $product_data->get_stock_quantity()), 'error');
            }
        }
    }

...
$single_form = PPOM() -> get_product_meta ( PPOM() -> productmeta_id );
...
if ( $single_form -> productmeta_style != '') {
	...
	echo '<div id="nm-productmeta-box-' . PPOM() -> productmeta_id . '" class="nm-productmeta-box">';
	...
	echo '<input type="hidden" id="_productmeta_id" value="'.PPOM() -> productmeta_id.'">';
	...
	foreach ( $existing_meta as $key => $meta ) {
		...
		switch ($type) {
			...
			case 'text':
					...
					$maxlength = isset($meta['max_length']) ? $meta['max_length'] : 9999;
					...
					PPOM() -> inputs[$type]	-> render_input($args, $data_default);					
				...
				case 'textarea':
					...
					$maxlength = isset($meta['max_length']) ? $meta['max_length'] : '';
					$args = array(	
							'name'			=> $name,
							'id'			=> $name,
							'data-type'		=> $type,
				...
				case 'select':
					...
					PPOM() -> inputs[$type]	-> render_input($args, $options, $default_selected);
				...
				case 'checkbox':
					...
					PPOM() -> inputs[$type]	-> render_input($args, $options, $defaul_checked);
					//for validtion message
					echo '<span class="errors"></span>';
					echo '</div>';
